...
# TODO: refactor this so it doesnt strictly require an item maybe?
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            self.engine.message_log.add_message(
                f"A lighting bolt strikes the {target.name} with an ear-splitting thunder, for {self.damage} damage!"
            )
...
class ConfusionConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target is consumer:
            raise Impossible("Confusing yourself probably isn't the best idea...")

        self.engine.message_log.add_message(
            f"The eyes of the {target.name} glaze over with a vacant look, and it starts to stumble around aimlessly!",
            color.status_effect_applied,
        )
        target.ai = Entities.Components.ai.ConfusedEnemy(
